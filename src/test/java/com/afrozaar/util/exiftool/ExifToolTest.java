...
public class ExifToolTest {
    ...
    @Test
    public void ExtractJsonNodeFromResponse() throws ExiftoolException {

        InputStream inputStream = getClass().getResourceAsStream("/json/exifresponse.json");

        final JsonNode jsonNode = exifTool.fromResponse(inputStream);
        final ObjectNode objectNode = exifTool.getObjectNode(jsonNode, 0);

        final Set<Profile> parsedProfiles = exifTool.getParsedProfiles(objectNode);

        for (Profile parsedProfile : parsedProfiles) {
            LOG.debug("parsedProfile: {}", parsedProfile);
        }
    }

    @Test
    public void FindValuesForSupportedTags() throws ExiftoolException {
        InputStream inputStream = getClass().getResourceAsStream("/json/exifresponse.json");
        final JsonNode jsonNode = exifTool.fromResponse(inputStream);
        final ObjectNode node = exifTool.getObjectNode(jsonNode, 0);

        final Map<SupportedTag, Collection<Object>> valuesForSupportedTags = exifTool.getValuesForSupportedTags(node, SupportedTag.values());

        assertThat(valuesForSupportedTags).isNotEmpty();

        System.out.println("valuesForSupportedTags = " + valuesForSupportedTags);
    }
